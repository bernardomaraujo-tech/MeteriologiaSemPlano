...
function applyDynamicBackground(weatherCode, isDay){
  ...
  const nightFactor = (Number(isDay) === 0) ? 0.85 : 1;
...
/* altura do topo: header + "Agora" + 20px */
...
function init(){
  ...
  // dropdown por ordem alfabética, mantendo Alcabideche como default e 1º
  const alc = LOCATIONS.find(l => l.id === "alcabideche") ?? LOCATIONS[0];
  const rest = LOCATIONS.filter(l => l.id !== alc.id).slice().sort((a,b) => a.name.localeCompare(b.name, "pt-PT"));

  const ordered = [alc, ...rest];
  ...
  // manter altura do topo certa em rotação/resize
  window.addEventListener("resize", () => updateSkyWindowHeight());

  refresh();
  setInterval(refresh, REFRESH_MS);

  // 2º passe para apanhar fontes/layout após render
  setTimeout(updateSkyWindowHeight, 250);
}
